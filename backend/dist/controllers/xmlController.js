    ...
    try {
        ...
        for (const item of Array.isArray(items) ? items : [items]) {
            ...
            if (name && quantity) {
                let categoryId = null;
                if (category) {
                    let categoryResult = await database_1.default.query('SELECT id FROM categories WHERE name = $1', [category]);
                    if (categoryResult.rows.length > 0) {
                        categoryId = categoryResult.rows[0].id;
                    }
                    else {
                        const newCategory = await database_1.default.query('INSERT INTO categories (name) VALUES ($1) RETURNING id', [category]);
                        categoryId = newCategory.rows[0].id;
                    }
                }
                await database_1.default.query('INSERT INTO items (name, category_id, quantity, notification_threshold) VALUES ($1, $2, $3, $4)', [name, categoryId, parseInt(quantity), 5]);
            }
        }
