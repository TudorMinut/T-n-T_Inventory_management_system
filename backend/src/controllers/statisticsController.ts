...
export const getStatsHtml = async (_req: IncomingMessage, res: ServerResponse) => {
    try {
        ...
        sendError(res, 500, "Eroare la generarea statisticilor HTML.");
    }
};
...
export const getStatsPdf = async (_req: IncomingMessage, res: ServerResponse) => {
    try {
        ...
        doc.moveDown();
        doc.fontSize(12);
        items.rows.forEach((item: { name: string; quantity: number; category: string | null }) => {
            doc.text(`Articol: ${item.name} | Cantitate: ${item.quantity} | Categorie: ${item.category || ""}`);
        });
        doc.end();
    } catch (error) {
        sendError(res, 500, "Eroare la generarea statisticilor PDF.");
    }
};
